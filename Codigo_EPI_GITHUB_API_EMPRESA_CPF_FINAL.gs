...
function doGet(e) {
  ...
  // API usada pelo aplicativo hospedado no GitHub Pages
  if (String(p.api || '') === '1') {
    ...
    try {
      ...
      if (action === 'carregarDados') {
        resultado = carregarDados();
      } else if (action === 'consultarFichaColaborador') {
        resultado = consultarFichaColaborador(p.nome || '');
      } else if (action === 'listarFichas') {
        resultado = listarFichas();
      } else if (action === 'listarSaidasRecentes') {
        resultado = listarSaidasRecentes(Number(p.limite || 30));
      } else if (action === 'listarSaidasPorData') {
        resultado = listarSaidasPorData(p.data || '');
      } else {
        resultado = { erro: 'Ação de API não encontrada.' };
      }
  ...
  // Abre a versão Google Apps Script normalmente
  return HtmlService
...
function doPost(e) {
  ...
  try {
    ...
    if (!payload) {
      throw new Error('Payload não recebido.');
    }
...
/****************************************************
 * API - HISTÓRICO PARA O GITHUB PAGES
 ****************************************************/
...
function listarSaidasRecentes(limite) {
  ...
  if (!sheet) {
    throw new Error('A aba SAIDA EPI não foi encontrada.');
  }
...
function listarSaidasPorData(dataTexto) {
  ...
  return obterSaidasHistorico_(sheet, 200, dataTexto || '');
}
